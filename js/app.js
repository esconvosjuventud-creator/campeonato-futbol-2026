(() => {
  ...
  function updateCountdown(){
    ...
    $("countdownBar").style.width=`${pct.toFixed(2)}%`;
    $("countdownPercent").textContent=`${Math.round(pct)}%`;

    if(now < START){
      const remaining=START.getTime()-now.getTime();
      setCountdownValues(remaining);
      $("countdownKicker").textContent="Próximamente";
      $("countdownTitle").textContent="Tiempo para que abran las inscripciones";
      $("countdownMessage").textContent="Las inscripciones abren el 26 de agosto de 2026 a las 00:00.";
      card.classList.remove("urgent","closed");
      return;
    }
  ...
  async function submit(){
    ...
    let draft=null;
    try{
      // Genera IDs nuevos en cada intento de envío para evitar colisiones
      // si un intento anterior quedó incompleto en Supabase.
      players = players.map(p => ({...p,id:uuid()}));
